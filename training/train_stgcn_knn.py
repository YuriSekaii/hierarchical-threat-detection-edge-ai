...
# --- k-NN OOD Hyperparameters ---
# Optimal values found via grid search on Fold 2:
#   k=2, pct=95 -> OOD reject=6/7, Violence detect=33/33
# k=1 is also good (pct=92), but k=2 gives more margin.
...
# NOTE: We do NOT L2-normalize features. Triplet-trained embeddings already have
# well-structured distances. L2-norm compresses the distance range on a unit
# hypersphere, making OOD samples appear artificially close to ID data.
...
# =======================================================================
# 1. DATASET DEFINITIONS (Identical to Train_Violence_STGCN.py)
# =======================================================================
...
# =======================================================================
# 2. MODEL (Identical to Train_Violence_STGCN.py)
# =======================================================================
...
# =======================================================================
# 3. HELPER FUNCTIONS (Training helpers identical, OOD functions changed)
# =======================================================================
...
def knn_score(query_features, feature_bank, k):
    """
    Compute the k-th nearest neighbor distance for each query against the feature bank.
    Uses raw (unnormalized) features to preserve magnitude-based OOD separation.
    
    Args:
        query_features: (Q, D) tensor — raw feature embeddings
        feature_bank:   (N, D) tensor — raw training feature embeddings
        k:              int — use the k-th nearest neighbor distance as the OOD score
    
    Returns:
        kth_dists: (Q,) tensor — distance to the k-th nearest neighbor for each query
    """
    # Clamp k to not exceed bank size
    effective_k = min(k, feature_bank.shape[0])
    
    # Compute pairwise L2 distances: (Q, N)
    dists = torch.cdist(query_features, feature_bank, p=2)
    
    # Get the k-th smallest distance for each query
...
def build_knn_feature_bank(dataset, model, device):
    ...
    This replaces calc_mahalanobis_param() — instead of computing mean + inv_cov,
    we keep the raw features as a lookup bank for k-NN.
    
    NOTE: We intentionally do NOT L2-normalize here. Sun et al. (2022) used
    L2-norm for ImageNet classifiers with huge magnitude variance across layers.
    Our triplet-trained STGCN features are already well-structured in metric
    space — normalizing compresses the distance range and hurts OOD rejection.
    
    Returns:
        feature_bank: (N, 256) tensor — raw training features
    """
    ...
    feature_bank = torch.cat(features_list)
...
def tune_knn_threshold(feature_bank, k, percentile=95):
    """
    Determine the OOD threshold by computing leave-one-out k-NN distances
    on the training set and taking the given percentile.
    
    For each training sample, we find its k-th nearest neighbor distance
    (excluding itself) and use the percentile of these distances as threshold.
    
    This replaces tune_ood_threshold() from the Maha version.
    
    Args:
        feature_bank: (N, 256) tensor — raw training features
        k:            int — k for k-NN
        percentile:   float — percentile for threshold (e.g. 95 means 95% of 
                       training samples will be classified as in-distribution)
    
    Returns:
        threshold: float — the OOD threshold
        train_knn_dists: numpy array — k-NN distances for all training samples
    """
    N = feature_bank.shape[0]
    effective_k = min(k + 1, N)  # +1 because self is always the nearest neighbor (dist=0)
    
    # Compute all pairwise distances
    dists = torch.cdist(feature_bank, feature_bank, p=2)  # (N, N)
    
    # For each sample, get (k+1)-th smallest distance (skip self which is distance 0)
    kth_dists, _ = dists.kthvalue(effective_k, dim=1)  # (N,)
...
def validate_category(category_name, folder_paths, model, feature_bank, threshold, k, device):
    ...
    Replaces the Maha-based validate_category().
    
    For each video's clips:
      1. Extract raw features (no L2-norm)
      2. Compute k-th NN distance to the training feature bank
      3. If ANY clip has knn_dist <= threshold → classify as Violence (in-distribution)
         Otherwise → classify as OOD
    """
    ...
    for src, clips in video_clips.items():
        ...
        with torch.no_grad():
            ...
            # Compute k-NN distance for each clip
            knn_dists = knn_score(feats, feature_bank, k)
            
            # If ANY clip is close enough to training data -> Violence
...
def main():
    ...
    # 2. BUILD k-NN FEATURE BANK FROM BEST FOLD
    model = STGCNModel(num_classes=1).to(device)
    model.load_state_dict(torch.load(best_model_path, weights_only=True))
    model.eval()
    
    print("\n--- Building k-NN Feature Bank on Training Data ---")
    ...
    # Use configured k (or auto-select if KNN_K == 0)
    if KNN_K == 0:
        effective_k = max(1, min(50, round(N_bank * 0.01)))
        print(f"Auto-selected k = {effective_k} (~1% of {N_bank} training clips)")
    else:
        effective_k = min(KNN_K, N_bank - 1)
        if effective_k != KNN_K:
            print(f"Note: Adjusted k from {KNN_K} to {effective_k} (dataset has {N_bank} samples)")
    
    print(f"\nTuning k-NN Threshold (k={effective_k}, {KNN_PERCENTILE}% ID retention)...")
    threshold_val, train_knn_dists = tune_knn_threshold(feature_bank, effective_k, percentile=KNN_PERCENTILE)
    print(f"k-NN Threshold: {threshold_val:.6f}")
    print(f"Train k-NN Dist Stats: min={np.min(train_knn_dists):.6f}, "
          f"mean={np.mean(train_knn_dists):.6f}, "
          f"max={np.max(train_knn_dists):.6f}, "
          f"std={np.std(train_knn_dists):.6f}")
    ...
    for cat, paths in categories.items():
        n, acc, prec, rec, f1, tp, tn, fp, fn = validate_category(cat, paths, model, feature_bank, threshold_val, effective_k, device)
    ...
    val_df.to_csv(os.path.join(log_output_dir, "validation_results_knn.csv"))
    print(f"Validation results saved to {os.path.join(log_output_dir, 'validation_results_knn.csv')}")
