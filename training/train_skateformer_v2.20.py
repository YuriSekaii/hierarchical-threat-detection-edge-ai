"""
SkateFormer Triplet Training with Partitioned Skeletal-Temporal Transformer - Version 2.20
Trains SkateFormer on scale-normalized skeleton action recognition data across 3 folds.
100% self-contained: uses local workspace datasets and modules. Zero external dependencies.
Exports champion checkpoint (weights/skateformer_violence_v2.20.pth) and training logs.
"""
...
def train_one_fold(
    ...
    print(f"\n--- Training Fold {fold_num} on {device} (SkateFormer v2.20, lr={lr}, embed_dim={embed_dim}, blocks={num_blocks}) ---", flush=True)
    ...
    save_path = os.path.join(model_dir, f"skateformer_violence_v2.20_fold{fold_num}.pth")
    torch.save(early_stopping.best_model_state, save_path)

    # Save training log
    df_hist = pd.DataFrame(history)
    log_csv = os.path.join(log_dir, f"training_log_skateformer_v2.20_fold{fold_num}.csv")
...
def main():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("=" * 80, flush=True)
    print("   TRAINING: SKATEFORMER PARTITIONED SPATIAL-TEMPORAL ViT (Version 2.20)", flush=True)
    ...
    for fold, (train_idx, valid_idx) in enumerate(gkf.split(full_dataset.samples, groups=groups)):
        ...
        path, val_loss, hist = train_one_fold(
            ...
            lr=1e-3, epochs=30, embed_dim=128, num_blocks=4, dropout=0.1,
    ...
    champion_target = os.path.join(model_output_dir, "skateformer_violence_v2.20.pth")
    shutil.copyfile(best_model_path, champion_target)
    print(f">> Champion Checkpoint Saved as: {champion_target}", flush=True)

    summary_path = os.path.join(tuning_dir, "training_summary_skateformer_v2.20.json")
