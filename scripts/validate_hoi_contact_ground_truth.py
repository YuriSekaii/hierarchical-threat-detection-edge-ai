"""
scripts/validate_hoi_contact_ground_truth.py

Official Ground Truth Evaluation Script for Version 4.30 (Contact-State HOI Transformer).
Evaluates Dual-Stream HOI Contact detector against all 715 Ground Truth surveillance images (data/Ground_Truth_Dataset).
...
OUTPUT_JSON_DEFAULT = os.path.join(REPO_DIR, "results", "ground_truth_benchmark", "hoi_contact_gt_validation_results.json")
...
def run_benchmark(
    ...
    for idx, img_path in enumerate(image_files):
        ...
        # Convert normalized GT to pixel coordinates
        gt_boxes = [xywhn2xyxy(b[1], b[2], b[3], b[4], w_img, h_img) for b in gt_raw]

        # Run dual-stream detector
        ...
        per_image_results.append({
            ...
            "detected_hands": pred["detected_hand_count"],
            "tp": len(matched_pred),
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Version 4.30 Contact-State HOI on Official Ground Truth Dataset")
    parser.add_argument("--weapon-conf", type=float, default=0.20, help="Weapon candidate proposal confidence threshold")
    parser.add_argument("--radius", type=float, default=2.2, help="Bipartite spatial interaction radius ratio")
    parser.add_argument("--contact-thresh", type=float, default=0.50, help="DINOv2 contact-state affinity threshold")
    ...
    print(f"\n" + "=" * 90)
    print(f"VERSION 4.30 OFFICIAL GROUND TRUTH BENCHMARK: CONTACT-STATE HOI TRANSFORMER")
    print(f"Dataset Root: {GT_DATASET_DIR} ({len(image_files)} test images)")
    print(f"Config: Proposal Conf={args.weapon_conf}, Interaction Radius={args.radius}, Contact Thresh={args.contact_thresh}, IoU={args.iou}")
    print("=" * 90 + "\n")
    ...
    summary_json = {
        ...
        "version": "4.30",
        "architecture": "Contact-State HOI Transformer (DINOv2 Dual-Stream)",
        "configuration": {
            "weapon_conf": args.weapon_conf,
            "interaction_radius": args.radius,
            "contact_thresh": args.contact_thresh,
