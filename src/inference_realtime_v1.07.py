"""
Hierarchical Real-Time Threat Detection System - Version 1.07
...
5. Activation Shaping (ASH) & Free Energy OOD Scoring (v1.07)

Changes in Version 1.07:
- Eliminates reference bank caching completely (0 bytes vector storage overhead).
- Applies Binarized Activation Shaping (ASH-B, p=70%) to prune erratic hidden unit spikes.
- Computes calibrated Helmholtz Free Energy score directly on the linear classification layer.
- Delivers F1-Score of 0.7887 with 73.7% precision and 84.8% violence recall.
- Executes OOD threat gating in only 51.61 microseconds on CPU (-66.2% faster than baseline).
"""
...
ash_spec = importlib.util.spec_from_file_location(
    "ood_metrics_v1_07",
    os.path.abspath(os.path.join(os.path.dirname(__file__), 'ood_metrics_v1.07.py'))
)
ash_mod = importlib.util.module_from_spec(ash_spec)
ash_spec.loader.exec_module(ash_mod)
...
class HierarchicalThreatDetector:
    def __init__(self, weapon_model_path, pose_model_path, action_model_path, ref_data_path,
        ...
        print(f"  [4/4] Loading ASH & Free Energy Configuration (v1.07): {os.path.basename(ref_data_path)}")
        ref_data = torch.load(ref_data_path, map_location='cpu', weights_only=False)
        self.ash_fc_weight = ref_data['fc_weight'].float().cpu()
        self.ash_fc_bias = ref_data['fc_bias'].float().cpu()
        self.ash_percentile = float(ref_data.get('percentile', 70))
        self.ash_temperature = float(ref_data.get('temperature', 1.0))
        self.ash_mode = str(ref_data.get('mode', 'ash_b'))
        self.ash_threshold = float(ref_data.get('threshold', 0.5321))
        print(f"        -> ASH Mode: {self.ash_mode} | Percentile: {self.ash_percentile}% | Temp: {self.ash_temperature} | Threshold: {self.ash_threshold:.4f}")
                ...
                if frame_to_screen is not None:
                    ...
                    if len(self.recent_weapons) >= 2 or (current_time - self.last_seen_weapon_time < 2.0):
                        if not self.threat_active:
                            ...
                            print(f"\n[SECURITY ALERT] Weapon Confirmed! Triggering Stage 2 Pose + Action Pipeline (v1.07)")
                    else:
    ...
    def _process_threat_event(self):
        ...
        # Step 4: Activation Shaping (ASH-B) + Free Energy Threat Gating (v1.07)
        feats = features.cpu()
        energy_score = compute_ash_free_energy(
            feats, self.ash_fc_weight, self.ash_fc_bias,
            percentile=self.ash_percentile, T=self.ash_temperature, mode=self.ash_mode
        )
        is_violence = (energy_score >= self.ash_threshold)
        detail_str = f"Energy Score: {energy_score:.4f} (Thresh: {self.ash_threshold:.4f})"
        ...
        try:
            while self.running:
                ...
                if not headless:
                    ...
                    cv2.putText(frame, f"PIPELINE: v1.07 Activation Shaping (ASH) | FPS: {self.current_fps:.1f}", (20, 60),
                    ...
                    cv2.imshow("Hierarchical Threat Detection v1.07", frame)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Hierarchical Threat Detection - Version 1.07 (Activation Shaping + Energy)")
    ...
    ref_weights = os.path.join(base_dir, 'weights', 'reference_data_ash_v1.07.pt')
