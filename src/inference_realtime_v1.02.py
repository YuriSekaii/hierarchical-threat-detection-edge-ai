"""
Hierarchical Real-Time Threat Detection System - Version 1.02
Dual-Stage Surveillance Architecture with:
1. In-Memory JPEG Buffer Compression (Option A, v1.01)
2. Torso-Scale Invariant Kinematic Normalization (v1.02)
3. Retrained ST-GCN + Calibrated Deep k-NN Manifold Gating (v1.02)

Changes in Version 1.02:
- Replaced centroid-only normalization with scale-invariant torso normalization (L_torso = ||mid_shoulder - mid_hip||).
- Motion vector amplitudes and action embeddings remain invariant to subject distance from camera.
- Loaded v1.02 ST-GCN trained checkpoint and calibrated Deep k-NN manifold reference bank.
- Preserved baseline (v1.00) and v1.01 files intact.
"""
...
class FrameItem:
    """Compressed Frame Storage in Circular Buffer (v1.01/v1.02).
...
class HierarchicalThreatDetector:
    def __init__(self, weapon_model_path, pose_model_path, action_model_path, ref_data_path,
        ...
        print(f"[INIT] Initializing Hierarchical Detector (v1.02 Scale-Invariant) on {self.device}...")
        ...
        # 2. Load ST-GCN Action Model (v1.02)
    ...
    def action_recognition_loop(self):
        """Thread 3: Zero-Drop gapless sliding-window kinematic evaluator with v1.02 Scale Normalization."""
        while self.is_running:
            now = time.time()
    ...
    def run(self, source=0, headless=False):
        ...
        print("[RUNNING] Threat detection system v1.02 started. Press 'q' to exit.")
        ...
        while cap.isOpened() and self.is_running:
            ...
            if not headless:
                ...
                cv2.imshow("Hierarchical Threat Detection (v1.02 Edge AI)", frame)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Hierarchical Edge-AI Threat & Violence Detection (v1.02)")
