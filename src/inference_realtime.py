...
class HierarchicalThreatDetector:
    def __init__(self, weapon_model_path, pose_model_path, action_model_path, ref_data_path, device='cuda'):
        ...
        print(f"[INIT] Loading Models onto {self.device}...")
    ...
    def action_recognition_loop(self):
        """Thread 3: On-demand kinematic action evaluation."""
        while self.is_running:
            ...
            if dist < self.knn_threshold:
                ...
                self.status_text = "WEAPON SEEN, PASSIVE MOTION"
                self.status_color = (0, 165, 255)  # Orange

            # Reset gate after window expires
            time.sleep(0.5)
            self.stage2_active = False

    def run(self, source=0):
        """Main thread: Video ingestion and GUI rendering."""
