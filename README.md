...
* **Compute Savings:** Reduces continuous idle inference workloads by **~90%**, reserving GPU/NPU compute until a weapon is verified (Confidence > 0.5).
...
* **Cross-Validation Result:** Fold 2 converged with optimal validation loss ({val} = 0.0177$) at epoch 19.
...
| Single-Action Baseline (52 Clips) | Mahalanobis Distance | 96.97% | 96.97% | 94.74% | 96.97% | 96.15% | Historical baseline on unimodal Cut-Down clips only (alidation_results_micro.csv). |
