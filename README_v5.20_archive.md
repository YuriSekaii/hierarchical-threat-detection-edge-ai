...
| :--- | :--- | :--- | :--- |
| **Stage 1 Weapon Detector** | Distilled YOLO26s (One-to-Many NMS) | **One-to-One Hungarian NMS-Free Distilled YOLO26s** | **Production Champion (86.88% F1, 12.86 ms)**; slashes seam/zipper false alarms by **>50%** (53 → 23 FP) |
| **Inference Geometry** | Square Letterboxing ($640 \times 640$) | **Dynamic Rectangular ($384 \times 640$)** | **Applied to BOTH Stage 1 & Stage 2a Pose**; eliminates 163,840 padding pixels (**-40.0% waste**), cuts stride audit by -127.6 ms |
...
│   ├── inference_production_pipeline.py    # [PRODUCTION] Unified multi-threaded surveillance orchestrator
│   ├── pipeline_buffer.py                  # In-memory TurboJPEG ring buffer storage (188.5 MB)
│   ├── stage1_detector.py                  # Stage 1 Hungarian NMS-Free Distilled YOLO26s detector
│   ├── stage2a_pose_tracker.py             # Stage 2a YOLO26s-Pose + ByteTrack threat actor guard
│   ├── stage2b_staircase.py                # Stage 2b Champion 2 Staircase Cascade action engine
...
### 2. Run Production Surveillance Pipeline (Real-Time Webcam / Video)
Executes end-to-end real-time surveillance using the modular pipeline (Distilled YOLO26s + TurboJPEG Buffer + ByteTrack + Champion 2 Staircase Cascade):
```bash
# Run on live webcam (camera index 0):
python src/inference_production_pipeline.py --source 0 --conf 0.45

# Run on a recorded video file:
python src/inference_production_pipeline.py --source /path/to/surveillance_stream.mp4 --conf 0.45

# Or test with simulated dry run (headless verification):
python src/inference_production_pipeline.py --dry-run
```

### 3. Run Offline Validation Suite Benchmark (Staircase Cascade v5.10)
Validates the Champion 2 Staircase Cascade on the standardized 77-video surveillance dataset:
```bash
python src/inference_staircase_cascade_v5.10.py --benchmark
```

### 4. Run Live Hardware CUDA Event Profiler (Champion 1 vs Champion 2)
...
### 5. Run Stage 1 NMS-Free Hungarian Detection
```bash
